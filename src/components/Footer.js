...
export default function Footer() {
  ...
  return (
    <footer
      ref={footerRef}
      className="relative isolate overflow-hidden px-6 py-16 text-white transition-all duration-700 ease-out md:px-20"
      style={{
    ...
    >
      ...
      <div className="relative mx-auto w-full max-w-6xl flex flex-col gap-12 text-white">
        <div className="grid gap-10 text-white md:grid-cols-3">
          <div
            className={`space-y-5 transition-all duration-700 ${reveal} text-white [&_*]:!text-white`}
          ...
          >
            ...
            <ul className="space-y-2 text-base text-white md:text-lg">
              {quickLinks.map(({ label, href }) => (
                <li key={label}>
                  <Link
                    href={href}
                    className="inline-flex items-center gap-2 font-medium text-white transition-colors duration-200 hover:text-accent hover:underline hover:decoration-accent/60 focus:outline-none focus-visible:ring-2 focus-visible:ring-secondary/60 focus-visible:ring-offset-2 focus-visible:ring-offset-base"
                  >
        ...
        <div
          className={`h-px w-full bg-white/30 transition-opacity duration-700 ${isVisible ? "opacity-100" : "opacity-0"}`}
          style={{ transitionDelay: "0.55s" }}
        />

        <div
          className={`text-center text-sm text-white/80 transition-all duration-700 ${reveal}`}
